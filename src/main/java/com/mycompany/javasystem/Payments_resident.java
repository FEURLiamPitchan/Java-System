...
public class Payments_resident {
    ...
    private void checkAndUpdatePendingPayments() {
        try {
            ...
            if (conn == null) {
                System.out.println("No database connection - skipping payment status check");
                return;
            }
            ...
            System.out.println("Database error in checkAndUpdatePendingPayments: " + e.getMessage());
        }
    }
    ...
    private void loadPayments(String search, String status) {
        ...
        try {
            Connection conn = DatabaseConnection.getConnection();
            if (conn == null) {
                Label empty = new Label("No database connection - running in demo mode.");
    ...
    private void updatePaymentStatus(String refNumber, String newStatus) {
        try {
            Connection conn = DatabaseConnection.getConnection();
            if (conn == null) {
                System.out.println("Database not available - cannot update payment status");
                return;
            }
    ...
    private void archivePayment(String refNumber) {
        try {
            Connection conn = DatabaseConnection.getConnection();
            if (conn == null) {
                System.out.println("Database not available - cannot archive payment");
                return;
            }
    ...
    private void createTestPayment() {
        try {
            ...
            if (conn == null) {
                ...
                error.setTitle("Database Not Available");
                error.setHeaderText("Running in Demo Mode");
                error.setContentText("Database connection not available. Cannot save payment records.");
                error.showAndWait();
                return;
            }
